...
// TODO this is kind of annoying. There must be a third party dependency that'd do this in a one-liner.
...
class Filesize extends Component {
  ...
  componentDidMount() {
    if (this.onUploadHandler){
      this.props.uploader.on('upload', this.onUploadHandler)
    }
  }
  ...
  componentWillUnmount() {
    if (this.onUploadHandler) {
      this.props.uploader.off('upload', this.onUploadHandler)
    }
  }
  ...
  render() {
    ...
    const { formattedSize, formattedUnits } = formatSizeAndUnits({ size: this.state.size, units: this.props.units })
